// Taken from github.com/tkanos/gonfig - modified the GetFromEnvVariables function
...
func GetFromEnvVariables(configuration interface{}) {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		// Recursively traverse all tested structs in config
		if f.Kind() == reflect.Struct {
			GetFromEnvVariables(f.Addr().Interface())
		}

		// check if we've got a field name override for the environment
		tagContent := t.Tag.Get(envTagName)
		value := ""
		if len(tagContent) > 0 {
			value = os.Getenv(tagContent)
		} else {
			value = os.Getenv(t.Name)
		}
		if len(value) == 0 {
			continue
		}
